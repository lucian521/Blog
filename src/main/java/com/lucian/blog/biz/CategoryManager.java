...
public class CategoryManager{
  ...
  public List<MapContainer> listAsTree(){
    ...
    for(int i = 1; i < preOrder.size(); i++){
      ...
      List<MapContainer> children = (List<MapContainer>) parent.putIfAbsent("nodes", new ArrayList<MapContainer>());
  ...
  private static MapContainer getLastParentByLevel(MapContainer mc, int currentlevel){
    ...
    for(int i = 1; i < currentlevel; i++){
      List<MapContainer> children = (List<MapContainer>) current.putIfAbsent("nodes", new ArrayList<MapContainer>());
